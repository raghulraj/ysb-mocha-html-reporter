...
module.exports = function(runner, options) {
    ...
    const reporterOpts = options["reporterOptions.output"] || {};
    const stats = this.stats;

    let result = '';

    runner.on('end', () => {
        const output = reporterOpts;
        const html = template({
            css: loadStylesheet(),
            js: loadScripts(),
            report: result,
    ...
    runner.on('pass', test => {
        ...
        const codeHTML = '<ul>' + ctx.reduce((html, item) => {
            return html + "<li>" + item + "</li>";
        }, "") + '</ul>';
        result += `<li class="test pass ${test.speed}">`;
        result += `<h2>${title}<span class="duration">${test.duration}ms</span></h2>`;
        if (ctx.length > 0) {
            result += `<pre><code>${code}</code><br><br><div id="divider" style="border:1px solid rebeccapurple;width: 100%;"></div><br><div id="logs" style="font-size: 14px;padding-bottom: 7px;"> Logs: </div><code style="display: block;white-space: pre-wrap;">${codeHTML}</code></pre>`;
        } else {
